...
import { 
  View, TextInput, FlatList, StyleSheet, TouchableOpacity, ActivityIndicator 
} from "react-native";
...
const Suggestions = () => {
  ...
  const startVoiceSearch = () => {
    if (!("webkitSpeechRecognition" in window)) {
      Toast.show({ type: "error", text1: "Voice search not supported in this browser." });
      return;
    }
  ...
  const searchHotelsByName = async (searchQuery) => {
    ...
    if (!/^[a-zA-Z0-9\s]+$/.test(query)) {
      Toast.show({ type: "error", text1: "Invalid hotel name. Use only letters and numbers." });
      return;
    }
    ...
    try {
      ...
      if (!response.data.length) {
        Toast.show({ type: "error", text1: "No hotels found matching the given name." });
      }
    } catch (error) {
      Toast.show({ type: "error", text1: "Error searching for hotels. Please try again." });
      console.error(error);
    }
    setLoading(false);
  };
    ...
    <Animatable.View animation="fadeInUp" style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        ...
        <TouchableOpacity style={styles.voiceButton} onPress={startVoiceSearch}>
          <MaterialIcons name="mic" size={24} color={isListening ? "red" : "#fff"} />
        </TouchableOpacity>
