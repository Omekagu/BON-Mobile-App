...
export default function SelectDateRange () {
  ...
  const getUserId = async () => {
    try {
      ...
      const response = await axios.get('http://10.0.1.20:5001/userData', {
  ...
  const handleBooking = async () => {
    if (!startDate || !endDate) {
      Toast.show({
        type: 'error',
        text1: 'Please select check-in and check-out dates'
      })
      return
    }
    try {
      ...
      if (!userId) {
        Toast.show({
          type: 'error',
          text1: 'User not logged in. Please log in first.'
        })
        return
      }
